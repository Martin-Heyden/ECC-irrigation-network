%Tradeoff in Fig 8 left

r_range_struct = [ 1 4 8 16 32 64 124, 248]
r_range_LQ = [0.1 0.2 0.5 1 2 4]
N = 10;
...
for i = 1:length(r_range_struct)
    ...
    struc_in_cost(i) =   sum_of_squares_diff(sys_S.input_trajectory);
end
for i = 1:length(r_range_LQ)
    contr_set =3;
    params_LQ.r = 0.0*ones(1,N);
    params_LQ.r_du = r_range_LQ(i)*ones(1,N);
    params_LQ.H = 200;		
    sys_LQ = simulate_system(N,sys_choice,contr_set,params_LQ,D,T);
    LQ_state_cost(i) = sum_of_squares(sys_LQ.level_trajectory);
    LQ_in_cost(i) =   sum_of_squares_diff(sys_LQ.input_trajectory);
end
